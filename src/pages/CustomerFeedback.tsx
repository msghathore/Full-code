...
import { mockFeedback, mockCustomers, calculateNPS } from '@/lib/mock-bi-data';
...
const CustomerFeedback = () => {
  ...
  // Calculate NPS and feedback analytics
  const npsScore = calculateNPS(mockFeedback);
  const totalFeedback = mockFeedback.length;

  const promoterCount = mockFeedback.filter(f => f.npsScore >= 9).length;
  const passiveCount = mockFeedback.filter(f => f.npsScore >= 7 && f.npsScore <= 8).length;
  const detractorCount = mockFeedback.filter(f => f.npsScore <= 6).length;

  const avgRating = mockFeedback.reduce((sum, f) => sum + f.rating, 0) / totalFeedback;

  // Service-specific feedback
  const serviceFeedback = mockFeedback.reduce((acc, feedback) => {
    if (!acc[feedback.service]) {
      acc[feedback.service] = { total: 0, sum: 0, count: 0 };
    }
    acc[feedback.service].total += feedback.rating;
    acc[feedback.service].sum += feedback.npsScore;
    acc[feedback.service].count += 1;
    return acc;
  }, {} as Record<string, { total: number; sum: number; count: number }>);

  const serviceData = Object.entries(serviceFeedback).map(([service, data]) => ({
    service,
    avgRating: Math.round((data.total / data.count) * 10) / 10,
    avgNPS: Math.round(data.sum / data.count),
  ...
  const feedbackTrends = mockFeedback.reduce((acc, feedback) => {
    ...
    if (!acc[date]) {
      acc[date] = { date, promoters: 0, passives: 0, detractors: 0, total: 0 };
    }
    acc[date].total += 1;
    if (feedback.npsScore >= 9) acc[date].promoters += 1;
    else if (feedback.npsScore >= 7) acc[date].passives += 1;
    else acc[date].detractors += 1;
    return acc;
  }, {} as Record<string, { date: string; promoters: number; passives: number; detractors: number; total: number }>);

  const trendData = Object.values(feedbackTrends)
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    .slice(-30)
    .map(item => ({
      date: new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
      nps: Math.round(((item.promoters - item.detractors) / item.total) * 100),
      avgRating: mockFeedback
        .filter(f => f.date === item.date)
        .reduce((sum, f) => sum + f.rating, 0) / item.total
    }));
  ...
  const recentFeedback = mockFeedback
    ...
    .map(feedback => ({
      ...feedback,
      customer: mockCustomers.find(c => c.id === feedback.customerId)
    }));
  ...
  const filteredFeedback = mockFeedback.filter(feedback => {
    ...
    const matchesService = filterService === 'all' || feedback.service === filterService;
  ...
  const ratingDistribution = [1, 2, 3, 4, 5].map(rating => ({
    ...
    percentage: Math.round((mockFeedback.filter(f => f.rating === rating).length / totalFeedback) * 100)
  }));
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold luxury-glow">{avgRating.toFixed(1)}</p>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold luxury-glow">{totalFeedback}</p>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold luxury-glow">78%</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="frosted-glass border-white/10">
                  <CardContent className="p-6">
                    ...
                    <div className="text-center mb-4">
                      <p className="text-3xl font-bold luxury-glow">{promoterCount}</p>
                      <p className="text-sm text-muted-foreground">
                        {Math.round((promoterCount / totalFeedback) * 100)}% of responses
                      </p>
                    </div>
                ...
                <Card className="frosted-glass border-white/10">
                  <CardContent className="p-6">
                    ...
                    <div className="text-center mb-4">
                      <p className="text-3xl font-bold luxury-glow">{passiveCount}</p>
                      <p className="text-sm text-muted-foreground">
                        {Math.round((passiveCount / totalFeedback) * 100)}% of responses
                      </p>
                    </div>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                ...
                <Card className="frosted-glass border-white/10">
                  <CardContent className="p-6">
                    ...
                    <div className="text-center mb-4">
                      <p className="text-3xl font-bold luxury-glow">{detractorCount}</p>
                      <p className="text-sm text-muted-foreground">
                        {Math.round((detractorCount / totalFeedback) * 100)}% of responses
                      </p>
                    </div>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                ...
                <CardContent>
                  ...
                  <div className="mt-6 p-4 bg-white/5 rounded-lg">
                    <p className="text-center text-lg">
                      NPS = (({promoterCount} - {detractorCount}) ÷ {totalFeedback}) × 100 = <span className={`font-bold ${getNPSScoreColor(npsScore)}`}>{npsScore}</span>
                    </p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <Select value={filterService} onValueChange={setFilterService}>
                      ...
                      <SelectContent className="frosted-glass border-white/20">
                        ...
                        <SelectItem value="Luxury Hair Styling">Luxury Hair Styling</SelectItem>
                      ...
                      {filteredFeedback.slice(0, 20).map((feedback) => {
                        ...
                        return (
                          <TableRow key={feedback.id}>
                            ...
                            <TableCell>{feedback.npsScore}/10</TableCell>
                            <TableCell>
                              <Badge className={getPromoterTypeColor(feedback.promoterType)}>
                                {feedback.promoterType}
                              </Badge>
                            </TableCell>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card className="frosted-glass border-white/10">
                  ...
                  <CardContent className="space-y-4">
                    {[
                      { issue: 'Wait times too long', frequency: '15%', impact: 'High' },
                      { issue: 'Staff communication', frequency: '12%', impact: 'Medium' },
                      { issue: 'Service pricing', frequency: '8%', impact: 'Medium' },
                      { issue: 'Facility cleanliness', frequency: '6%', impact: 'Low' }
                    ].map((item, index) => (
                ...
                <Card className="frosted-glass border-white/10">
                  ...
                  <CardContent className="space-y-4">
                    {[
                      { strength: 'Staff expertise and friendliness', frequency: '28%', impact: 'High' },
                      { strength: 'Service quality and results', frequency: '22%', impact: 'High' },
                      { strength: 'Relaxing atmosphere', frequency: '18%', impact: 'Medium' },
                      { strength: 'Product recommendations', frequency: '15%', impact: 'Medium' }
                    ].map((item, index) => (
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-blue-500/10 rounded-lg">
                      <h4 className="font-semibold text-blue-400 mb-2">Immediate Actions (0-30 days)</h4>
                    ...
                    <div className="p-4 bg-purple-500/10 rounded-lg">
                      <h4 className="font-semibold text-purple-400 mb-2">Long-term Strategy (30-90 days)</h4>
                      <ul className="text-sm space-y-1 text-muted-foreground">

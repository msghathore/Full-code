...
const RevenueTracking = () => {
  ...
  const { revenueData, kpis, loading, error } = useRevenueData(timeRange);

  // Process data for charts when data is available
  const monthlyData = revenueData.length > 0 ? revenueData.reduce((acc, day) => {
  ...
  const processedMonthlyData = Object.values(monthlyData).map(item => ({
    ...item,
  ...
  const serviceRevenue = revenueData.length > 0 ? revenueData.reduce((acc, day) => {
    acc.services += day.servicesRevenue;
    acc.products += day.productsRevenue;
    return acc;
  }, { services: 0, products: 0 }) : { services: 0, products: 0 };

  const revenueBreakdown = (serviceRevenue.services + serviceRevenue.products) > 0 ? [
    ...
    { name: 'Services', value: 0, color: '#8884d8', percentage: 0 },
    { name: 'Products', value: 0, color: '#82ca9d', percentage: 0 }
  ];

  // Profitability analysis
  const profitabilityData = processedMonthlyData.map(month => ({
    ...month,
  ...
  // Budget vs Actual (mock comparison - could be enhanced with real budget data)
  const budgetData = processedMonthlyData.map(month => ({
    month: month.month,
    actual: month.revenue,
    budget: month.revenue * 0.95, // Assume 5% under budget
    variance: month.revenue > 0 ? (month.revenue - month.revenue * 0.95) / (month.revenue * 0.95) * 100 : 0
  }));

  // Cash flow analysis
  const cashFlowData = processedMonthlyData.map((month, index) => ({
    month: month.month,
    inflow: month.revenue,
    outflow: month.revenue * 0.4, // Operating expenses
    netCashFlow: month.revenue * 0.6,
    cumulative: processedMonthlyData.slice(0, index + 1).reduce((sum, m) => sum + (m.revenue * 0.6), 0)
  }));
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold luxury-glow">{formatCurrency(kpis?.lifetimeValue || 0)}</p>
                    <p className="text-xs text-muted-foreground">CAC: {formatCurrency(kpis?.customerAcquisitionCost || 0)}</p>
                  </div>
                ...
                <Card className="frosted-glass border-white/10">
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                      ...
                      <span className="font-medium">{formatCurrency((kpis?.totalRevenue || 0) * 0.3)}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                    ...
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                      ...
                      <span className="font-medium">{formatCurrency((kpis?.totalRevenue || 0) * 0.2)}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="pt-4 border-t border-white/10">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatCurrency((kpis?.totalRevenue || 0) * 0.35)}</span>
                      </div>
                ...
                <CardContent>
                  <ChartContainer config={{}} className="h-[400px]">
                    <LineChart data={[
                        ...
                        { month: 'Dec 2024', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.08 || 0 },
                        { month: 'Jan 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.12 || 0 },
                        { month: 'Feb 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.15 || 0 },
                        { month: 'Mar 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.18 || 0 },
                        { month: 'Apr 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.20 || 0 },
                        { month: 'May 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.22 || 0 }
                      ]}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="month" />
                        <YAxis tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`} />
                        ...
                        <Line
                          ...
                          strokeWidth={3}
                          name="Historical"
                          strokeDasharray={processedMonthlyData.length > 0 ? "0" : "5 5"}
                        />
                        <Line
                          type="monotone"
                          dataKey="revenue"
                          ...
                          name="Forecast"
                          strokeDasharray="5 5"
                          data={processedMonthlyData.length > 0 ? [
                            processedMonthlyData[processedMonthlyData.length - 1],
                            { month: 'Dec 2024', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.08 },
                            { month: 'Jan 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.12 },
                            { month: 'Feb 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.15 },
                            { month: 'Mar 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.18 },
                            { month: 'Apr 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.20 },
                            { month: 'May 2025', revenue: processedMonthlyData[processedMonthlyData.length - 1].revenue * 1.22 }
                          ].slice(1) : []}
                        />
                      </LineChart>
                  </ChartContainer>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold luxury-glow">
                        {formatCurrency(processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.08 || 0)}
                      </p>
                ...
                <Card className="frosted-glass border-white/10">
                  <CardContent className="p-6">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold luxury-glow">
                        {formatCurrency(processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.15 || 0)}
                      </p>
                      <p className="text-xs text-muted-foreground">Next month projection</p>
                    </div>
                  </CardContent>
                ...
                <Card className="frosted-glass border-white/10">
                  <CardContent className="p-6">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold luxury-glow">
                        {formatCurrency(processedMonthlyData[processedMonthlyData.length - 1]?.revenue * 1.22 || 0)}
                      </p>
                      <p className="text-xs text-muted-foreground">Next month projection</p>
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-3 bg-white/5 rounded-lg">
                        ...
                        <p className="text-lg font-bold">{kpis?.profitMargin || 0}%</p>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg">
                        <p className="text-xs text-muted-foreground">ROI</p>
                        <p className="text-lg font-bold">{kpis?.roi || 0}%</p>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg">
                        <p className="text-xs text-muted-foreground">CAC</p>
                        <p className="text-lg font-bold">{formatCurrency(kpis?.customerAcquisitionCost || 0)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-white/5 rounded-lg">
                        ...
                        <p className="text-lg font-bold">{formatCurrency(kpis?.lifetimeValue || 0)}</p>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg">
                        <p className="text-xs text-muted-foreground">Retention Rate</p>
                        <p className="text-lg font-bold">{kpis?.retentionRate || 0}%</p>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg">
                        <p className="text-xs text-muted-foreground">Churn Rate</p>
                        <p className="text-lg font-bold text-red-400">{kpis?.churnRate || 0}%</p>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg">

...
// Generate time slots from 8 AM to 12 AM (16 hours) in 30-minute intervals
...
// Mock staff data - replace with real database queries
const mockStaff: StaffMember[] = [
  { id: '1', name: 'Sarah Johnson', username: 'sarah', password: 'staff2024', role: 'senior', specialty: 'Hair Styling', status: 'available', access_level: 'full' },
  { id: '2', name: 'Mike Chen', username: 'mike', password: 'staff2024', role: 'senior', specialty: 'Nails', status: 'busy', access_level: 'full' },
  { id: '3', name: 'Emma Davis', username: 'emma', password: 'staff2024', role: 'junior', specialty: 'Massage', status: 'available', access_level: 'limited' },
  { id: '4', name: 'Alex Rivera', username: 'alex', password: 'staff2024', role: 'junior', specialty: 'Facials', status: 'break', access_level: 'basic' }
];
...
const AdvancedSchedulingGrid = () => {
  ...
  const handleDrop = (e: React.DragEvent, targetDate: string, targetTime: string, targetStaffId: string) => {
    e.preventDefault();
      ...
      <div className="min-h-screen bg-gray-100 text-gray-800 font-sans">
          ...
          <div className="flex justify-around py-2 border-t border-gray-200">
            {weekDays.map(day => (
              <div key={day.toString()} className="text-center">
                ...
                <p className={`font-semibold text-lg ${isSameDay(day, selectedDate) ? 'text-blue-600' : ''}`}>
                  {format(day, 'd')}
                </p>
              </div>
            ))}
        ...
        <main className="flex flex-1 overflow-hidden">
          ...
          <div className="w-16 flex-shrink-0">
            {timeSlots.map(slot => (
              <div key={slot.time} className="h-12 text-right pr-2 text-xs text-gray-500 border-r border-gray-200">
          ...
          <div ref={gridRef} className="flex-1 grid grid-cols-3 gap-px bg-gray-200">
            {staff.map(staffMember => (
              <div
                ...
                // onDrop={(e) => handleDrop(e, format(selectedDate, 'yyyy-MM-dd'), /* need time */, staffMember.id)}
              >
                ...
                <div className="relative h-full">
                  {getAppointmentsForSlot(staffMember.id, selectedDate, 'all').map(appointment => {
                    const top = ((parseInt(appointment.appointment_time.slice(0, 2)) - 8) * 60 + parseInt(appointment.appointment_time.slice(3, 5))) / 60 * 48;
                    const height = (appointment.services?.duration_minutes || 60) / 60 * 48;
                    ...
                    return (
                      <div
                        ...
                        className="absolute left-2 right-2 p-2 rounded-lg shadow-md cursor-pointer text-white"
                      >
                        <p className="font-semibold text-sm">{appointment.full_name}</p>
                        <p className="text-xs">{appointment.services?.name}</p>
                      </div>
                    );
                  })}
                </div>

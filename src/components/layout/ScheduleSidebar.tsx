...
const ScheduleSidebar: React.FC<ScheduleSidebarProps> = ({
  ...
  const [localSelectedEmployees, setLocalSelectedEmployees] = useState<string[]>(selectedEmployees);
  const [localSelectedResources, setLocalSelectedResources] = useState<string[]>(selectedResources);

  const handleEmployeeToggle = (employeeId: string) => {
    const isCurrentlySelected = localSelectedEmployees.includes(employeeId);
    const newSelection = isCurrentlySelected
      ? localSelectedEmployees.filter(id => id !== employeeId)
      : [...localSelectedEmployees, employeeId];
  ...
  const handleResourceToggle = (resourceId: string) => {
    const isCurrentlySelected = localSelectedResources.includes(resourceId);
    const newSelection = isCurrentlySelected
      ? localSelectedResources.filter(id => id !== resourceId)
      : [...localSelectedResources, resourceId];
    
    setLocalSelectedResources(newSelection);
  ...
  // Use active selection - external props take precedence
  const activeSelectedEmployees = selectedEmployees.length > 0 ? selectedEmployees : localSelectedEmployees;
  const activeSelectedResources = selectedResources.length > 0 ? selectedResources : localSelectedResources;
          ...
          {employees.map((employee) => (
            <div key={employee.id} className="flex items-center space-x-3">
              <Checkbox
                ...
                checked={activeSelectedEmployees.includes(employee.id)}
          ...
          {resources.map((resource) => (
            <div key={resource.id} className="flex items-center space-x-3">
              <Checkbox
                ...
                checked={activeSelectedResources.includes(resource.id)}

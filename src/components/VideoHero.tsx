...
export const VideoHero = () => {
  ...
  return <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* Video Controls */}
      <div ref={controlsRef} className="absolute bottom-12 right-12 z-20">
        <button onClick={togglePlay} className="group flex items-center space-x-3 px-6 py-3 frosted-glass border border-white/20 rounded-full hover:border-white/40 transition-all cursor-hover">
          {isPlaying ? <Pause className="w-5 h-5 text-white group-hover:luxury-glow transition-all" /> : <Play className="w-5 h-5 text-white group-hover:luxury-glow transition-all" />}

...
import { Calendar, Clock, User, Phone, Mail, DollarSign, FileText, RotateCcw } from 'lucide-react';
...
interface RebookAppointmentDialogProps {
  isOpen: boolean;
  onClose: () => void;
  appointment: Appointment | null; // The original appointment to rebook
  staff: StaffMember[];
  onSave: (newAppointment: Appointment) => void; // Create new appointment
}

const RebookAppointmentDialog: React.FC<RebookAppointmentDialogProps> = ({
  isOpen,
  onClose,
  appointment,
  staff,
  ...
  useEffect(() => {
    if (appointment) {
      setFormData({
        ...
        appointment_date: '', // Clear date - user needs to select new slot
        appointment_time: '', // Clear time - user needs to select new slot
              ...
              <div>
                ...
                <Select value={formData.status} onValueChange={(value) => setFormData(prev => ({ ...prev, status: value }))}>
                  ...
                  <SelectContent className="bg-white border-gray-200">
                    ...
                    <SelectItem value="accepted" className="text-black">Accepted</SelectItem>
                  </SelectContent>
                </Select>
              </div>
...
export default RebookAppointmentDialog;
      ...
      {!hideWhenPopup && (
        ...
        <div className="container mx-auto px-4 md:px-8 flex items-center justify-between relative z-10">
           ...
           <Link to="/" className={`cursor-hover absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 ${shouldHideNavigationLogo ? 'hidden' : ''}`}>
             <div ref={logoRef} className="transition-all duration-500 text-center">
               <div className="text-3xl sm:text-4xl md:text-5xl font-serif font-light text-white logo-main md:mt-6">
                   <span
                     className={`main-logo-text ${(isHomePage && isOnBanner) ? 'text-6xl sm:text-8xl md:text-9xl' : 'text-3xl sm:text-4xl md:text-5xl'} text-white luxury-glow animate-glow-pulse inline-block text-hover-shimmer`}
                     style={(isHomePage && isOnBanner) ? { transform: 'scale(1.3) translateY(20px)', transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' } : { transform: 'scale(1.1) translateY(5px)', transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)', filter: 'drop-shadow(0 2px 3px rgba(0,0,0,0.3))' }}
                   >
                     ZAVIRA
                   </span>
               </div>
           ...
           {isSignedIn ? (
             ...
             <a
               href="/auth"
               className="text-white font-semibold hover:luxury-glow transition-all duration-300 cursor-hover text-sm md:text-base bg-black/50 backdrop-blur-sm px-4 py-2 rounded-full border border-white/30 hover:bg-black/70 hover:border-white/50"
             >
               {t('login')}
             </a>
           )}

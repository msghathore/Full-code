...
|-------------|--------------|-------|
| **Cormorant Gainsmond** | `font-serif` | Headings, luxury text |
...
## 🔍 CRITICAL: Research Methodology (ALWAYS FOLLOW)

When researching ANY topic, Claude MUST:

1. **Check Today's Date First**
   - Always verify the current date using web search
   - Use this date as the reference point for "latest" information

2. **Search for Up-to-Date Information**
   - Always search for the LATEST information available
   - Use current year in search queries (e.g., "React best practices 2025")
   - Do NOT rely on outdated knowledge or cached information

3. **Minimum 10 Sources Rule**
   - For any research task, consult AT LEAST 10 different sources
   - Sources must be dated as close to the current date as possible
   - Prioritize official documentation, reputable blogs, and recent articles

4. **Source Quality Criteria**
   - Official documentation (highest priority)
   - GitHub repos with recent activity
   - Stack Overflow answers from the last 6 months
   - Dev.to, Medium, or other tech blogs from current year
   - Conference talks or tutorials from recent events

5. **Always Cite Sources**
   - Provide links to the sources used
   - Note the date of each source when relevant
   - If information conflicts between sources, mention the discrepancy

6. **Technology-Specific Updates**
   - For npm packages: Check npm for latest version
   - For APIs: Check official docs for current endpoints
   - For frameworks: Look for recent changelogs and migration guides
